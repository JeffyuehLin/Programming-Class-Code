        ...
        for(i=0;i<total-1;i++)  //bubble-sort
        		for(j=0;j<total-1-i;j++){
        				if(aver[j]<aver[j+1]){
        						...
        						temp2=no[j];   no[j]=no[j+1];     no[j+1]=temp2;
        						temp3=math[j]; math[j]=math[j+1]; math[j+1]=temp3;
								temp4=eng[j];  eng[j]=eng[j+1];   eng[j+1]=temp4;
								temp5=prog[j]; prog[j]=prog[j+1]; prog[j+1]=temp5;
						}
				}
